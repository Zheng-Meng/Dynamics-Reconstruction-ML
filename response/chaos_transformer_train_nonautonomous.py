...
save_file_name = 'train_nonautonomous'
...
parser.add_argument('--num-epochs', default=50, type=int, help='Epochs') # default 50
...
parser.add_argument('--num-repeats', default=10, type=int, help='Number of times to repeat training per setup')
...
attractors_nonautonomous = ['forced_lorenz', 'forced_rossler', 'mega_extreme']

attractors_test = ['memristive_laser']
...
def train_and_evaluate(train_set, mask_ratios):
    ...
    model.eval()
    test_losses = np.zeros((len(attractors_test), len(mask_ratios)))
    with torch.no_grad():
        for mask_ratio_i in range(len(mask_ratios)):
            ...
            for file_i in range(len(attractors_test)):
                ...
                for data in test_loader:
                    ...
                    # random_seq_length = random.randint(args.seq_start, args.sequence_length)
                    inputs = inputs[:, :args.sequence_length, :]
                ...
                test_losses[file_i, mask_ratio_i] = avg_loss
...
group_names = ['with_nonautonomous', 'without_nonautonomous']
group_results = {}

for group in group_names:
    print(f"\n==== Training for {group} ====")
    test_loss_group = np.zeros((len(attractors_test), len(mask_ratios), args.num_repeats))

    for repeat in range(args.num_repeats):
        if group == 'with_nonautonomous':
            train_subset = random.sample(attractors_all, 7) + attractors_nonautonomous
        else:
            train_subset = random.sample(attractors_all, 10)

        _, test_losses = train_and_evaluate(train_subset, mask_ratios)
        test_loss_group[:, :, repeat] = test_losses

    group_results[group] = test_loss_group

# Save the results
os.makedirs('./save_results', exist_ok=True)
save_path = f'./save_results/performance_with_vs_without_nonautonomous_laser1_iter0.pkl'
with open(save_path, 'wb') as f:
    pickle.dump({'mask_ratios': mask_ratios, 'results': group_results}, f)
